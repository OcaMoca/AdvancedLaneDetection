...
bool CameraCalibration::add_chessboard_points(vector<string>& camera_images, cv::Size& board_size)
{
    ...
    for(int i = 0; i < camera_images.size(); i++)
    {
        ...
        found = findChessboardCorners(image, board_size, image_corners, CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_FILTER_QUADS);
...
double CameraCalibration::calibration(Size& image_size)
{
    ...
    return calibrateCamera(object_points, image_points, image_size, camera_matrix, dist_coefs, rvecs, tvecs, flag); 
}

void CameraCalibration::set_flag(bool radial, bool tangential)
{
    flag = 0;
    if(!tangential) flag += CALIB_ZERO_TANGENT_DIST;
    if(!radial) flag += CALIB_RATIONAL_MODEL;
}
